...
function init (doc) {
  ...
  // [MyBricks.ai]
  const targetDOM = document.querySelector('#_mybricks-geo-webview_')?.shadowRoot
  if (!targetDOM) return
  ...
  // [MyBricks.ai]
  targetDOM.appendChild(taroStyle)
...
// [MyBricks.ai] 导出 resetInteraction
export { disableAlertBeforeUnload, enableAlertBeforeUnload, hideLoading, hideToast, resetInteraction, showActionSheet, showLoading, showModal, showToast }
